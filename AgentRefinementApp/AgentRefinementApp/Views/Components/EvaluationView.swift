...
struct EvaluationView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("⭐ 評価").font(.system(size: 14, weight: .bold)).foregroundStyle(.secondary)
                ...
                ForEach(evaluations) { eval in
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            HStack {
                                Text(eval.evaluatorRole.rawValue.uppercased()).font(.system(size: 15, weight: .bold))
                                Spacer()
                                Text("\(eval.score)/10").font(.system(size: 16, weight: .bold))
                                    .foregroundStyle(scoreColor(eval.score))
                            }
                            if let comment = eval.comment, !comment.isEmpty {
                                Text(comment).font(.system(size: 15)).foregroundStyle(.secondary)
                            }
                        }
                    }
                    .padding(8)
            ...
            Text("新規評価").font(.system(size: 16, weight: .semibold)).foregroundStyle(.secondary)
            ...
            HStack {
                Text("スコア: \(score)/10").font(.system(size: 16))
            ...
            Button {
                ...
                Text("評価を送信").font(.system(size: 16, weight: .semibold))
